...
<%
    ...
    String strFaqIdSearch     = checkNull(request.getParameter( "FAQ_ID_SEARCH" ));
    String strFaqSubjSearch   = getField(request.getParameter( "FAQ_SUBJ_SEARCH" ));
    String strFaqDescSearch   = getField(request.getParameter( "FAQ_DESC_SEARCH" ));
    String strFaqAnswerSearch = getField(request.getParameter( "FAQ_ANSWER_SEARCH" ));

    String  strFaqIdData   = checkNull(request.getParameter( "FAQ_ID_TEMP" ));
    String  strFaqSubjData = getField(request.getParameter( "FAQ_SUBJ_TEMP" ));
    ...
    if( strMode.equals("DELETE") ) {
        ...
        con.addData( "ACTION_FLAG",  "String", "DF" );
        con.addData( "CURRENT_DATE", "String", strCurrentDate);
		
        bolnSuccess = con.executeService( strContainerName, strClassName, "deleteFaqManager" );
        if( !bolnSuccess ){
            strmsg  = "showMsg(0,0,\" " + lc_can_not_delete_faq + "\")";
            strMode = "SEARCH";
        }else{
            strmsg  = "showMsg(0,0,\" " + lc_delete_faq_successfull + "\")";
            strMode = "SEARCH";
        }
    }
    ...
    if( strMode.equals("SEARCH") ) {
        ...
        bolnSuccess = con.executeService( strContainerName, strClassName, "selectAllFaqManager" );
        if( bolnSuccess ) {
    ...
    if( strMode.equals("FIND") ) {
    	...
    	if( !strFaqIdSearch.equals("") ) {
    		con.addData( "FAQ_ID", "String", strFaqIdSearch);
    	}
    	if( !strFaqSubjSearch.equals("") ) {
        	con.addData( "FAQ_SUBJ", "String", strFaqSubjSearch);
    	}
    	if( !strFaqDescSearch.equals("") ) {
        	con.addData( "FAQ_DESC", "String", strFaqDescSearch);
    	}
    	if( !strFaqAnswerSearch.equals("") ) {
        	con.addData( "FAQ_ANSWER", "String", strFaqAnswerSearch);
    	}
    	if( sortfield.equals("") ) {
            sortfield="0";
        }
        ...
        bolnSuccess = con.executeService( strContainerName, strClassName, "findFaqManager" );
        if( bolnSuccess ) {
            strTotalPage = con.getHeader( "PAGE_COUNT" );
            strTotalSize = con.getHeader( "TOTAL_RECORD" );
            if( !strTotalSize.equals("0") ) {
...
function buttonClick( lv_strMethod, lv_strValue ){
    switch( lv_strMethod ){
        case "pInsert" :
            $("#formadd").attr('action', "faq2.jsp");
            $("#formadd #FAQ_ID_KEY").val("");
            $("#formadd #MODE").val("pInsert");
            $("#formadd").submit();
            break;
        case "search" :
            $("#form1").attr('action', "faq3.jsp");
            $("#form1").submit();
            break;
        case "pEdit" :
            $("#formadd").attr('action', "faq2.jsp");
            $("#formadd #FAQ_ID_KEY").val(lv_strValue.getAttribute("FAQ_ID"));
        ...
        case "DELETE" :
            ...
            $("#form1 #FAQ_ID_TEMP").val(lv_strValue.getAttribute("FAQ_ID"));
            $("#form1 #FAQ_SUBJ_TEMP").val(lv_strValue.getAttribute("FAQ_SUBJ"));
            $("#form1").submit();
            break;
    }
}
...
%>
                                        <tr class="hd_table"> 
                                            ...
                                            <td width="110" align="center">
                                                <div align="center"><span id="lb_faq_code"></span><%=sortimg[1]%></div>
                                            </td>
                                            <td align="center" width="480">
                                                <div align="center"><span id="lb_faq_subj"></span><%=sortimg[2]%></div>
                                            </td>
...
<%
    if( bolnSuccess ) {
        ...
        while( con.nextRecordElement() ) {
            ...
            strScript    = "FAQ_ID=\"" + strFaqIdData + "\" FAQ_SUBJ=\"" + strFaqSubjData + "\" ";

            idx = 2 - (intSeq % 2);
%>              
                                        <tr class="table_data<%=idx%>"> 
                                            <td width="10">&nbsp;</td>
                                            <td width="110"><div align="center"><%=strFaqIdData %></div></td>
                                            <td width="480"><div align="left"><%=strFaqSubjData.replaceAll( "##", "\r\n" ) %></div></td>
                                            <td colspan="3">
...
<input type="hidden" id="FAQ_SUBJ_TEMP" name="HEADER_TEMP"  value="">
...
<input type="hidden" id="FAQ_ID_KEY" name="FAQ_ID_KEY" value="">
<input type="hidden" id="screenname" name="screenname" value="<%=screenname%>">
