...
<%
	...
	String strProjectCode = "" , strProjectName = "";
	...
	if( strField1.length() != 0 ){
		con.addData( "PROJECT_NAME" , 	"String" , strField1 );
	    ...
	    if( strProjectFlag.length() != 0 ){
			if( strProjectFlag.equals("2")){
				bolnSuccess = con.executeService( strContainerName, "PROJECT_MANAGER", "findProjectManagerSearchPublic" );
	    	}else {
	    		bolnSuccess = con.executeService( strContainerName, "PROJECT_MANAGER", "findProjectManagerSearchPrivate" );
	    	}
		}else{
			bolnSuccess = con.executeService( strContainerName , "PROJECT_MANAGER" , "findProjectManager" );
		}
		if(bolnSuccess){
...
function submitForm(){
	form1.method = "post";
	form1.action = "zoom_project_manager.jsp";
	form1.submit();
}

function window_onload(){
    
    lb_cabinet_code.innerHTML = lbl_cabinet_code;
    lb_cabinet_name.innerHTML = lbl_cabinet_name;
...
<%
		...
		String[] sortImg = new String[2]; 	

		if(bolSuccessData){
			if(strSortField.equals("")){
				sortImg[0] = "<img src=\"../images/updown.gif\" width=20 height=11 border=0 style=\"cursor:pointer;\" onclick=\"click_sort('0','ASC')\">";		
...
%>        
          <td width="100" height="22" class="hd_table"><span id="lb_cabinet_code"></span>&nbsp;<%=sortImg[0] %></td>
          <td width="210" height="22" class="hd_table"><span id="lb_cabinet_name"></span>&nbsp;<%=sortImg[1] %></td>
        </tr>
      </table></td>
    </tr>
    <tr>
...
<%				
			...
			while( con.nextRecordElement() ){
				...
				strScript = " COLUMN_CODE=\"" + strProjectCode + "\"";
				strScript += " COLUMN_NAME=\"" + strProjectName + "\"";
...
%>
		  <tr class="table_data1" <%=strScript%> align="left">
		    <td width="100" height="22" style="padding-left: 5px"><%=strProjectCode%></td>
		    <td width="210" height="22"><%=strProjectName%></td>
		  </tr>
<%
				}else{
%>
