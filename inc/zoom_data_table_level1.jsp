...
<%
	...
	String strProjCode 	 = getField( request.getParameter( "PROJECT_CODE" ) );
	...
	if( strField1.length() != 0 ){
		...
		if( !strProjCode.equals("") ) {
			con.addData( "PROJECT_CODE", "String", strProjCode );
		}
		if(strTable.equals("USER_LEVEL")){
			con.addData( "FIELD_CODE" , "String" , "LEVEL_NAME" );			
		} else{
			con.addData( "FIELD_CODE" , "String" , strTable + "_NAME" );
		}
		
		if(strSortField.equals("2")&&strTable.equals("USER_LEVEL")){
			con.addData( "SORT_FIELD" , "String" , "3" );
		} else{
		...
		bolnSuccess = con.executeService( strContainerName , "ZOOM_TABLE_MANAGER" , "zoomDataTable1" );
		if(bolnSuccess){
...
function submitForm(){
	form1.method = "post";
	form1.action = "zoom_data_table_level1.jsp";
	form1.submit();
}
