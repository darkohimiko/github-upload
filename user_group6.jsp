...
<%
    ...
    String strConDocumentData  = getField( request.getParameter("hidConDocument" ) );
    ...
    if(strAccessTypeKey.equals("")){
            strAccessTypeKey = "A";
    }

    if(strAccessDocTypeKey.equals("")){
            strAccessDocTypeKey = "A";
    }
    ...
    if( strMode.equals("UPDATE") ) {
    	...
    	con.addData( "CON_DOCUMENT",    "String", strConDocumentData );
		...
		bolnSuccess = con.executeService( strContainerName, strClassName, "updateGroupAccessByOwner" );
		if( !bolnSuccess ) {
    ...
    if( strMode.equals("SEARCH") ) {
        ...
        if( strAccessTypeKey.equals("O") ) {
	        bolnSuccess = con.executeService( strContainerName, strClassName, "findAccessTypeUserGroup" );
	        
        }

        if( strAccessDocTypeKey.equals("O") ) {
	        con3.addData( "PROJECT_CODE", "String", strProjectCodeKey );
	        bolnSuccess3 = con3.executeService( strContainerName, strClassName, "findAccessDocTypeUserGroup" );
...
function concatAllValueUpdate() {
    ...
    if( obj_hidDocumentFlag.value == "T" ) {
        ...
        obj_hidConDocument.value = strConDocument;
    }
...
function buttonClick( lv_strMethod ) {
	switch( lv_strMethod ){
		case "save" :
			...
			if( (obj_hidAccessType.value == "O") && (obj_hidConDocument.value == "") ) {
...
function openZoom( lv_value ) {
    ...
    strUrl         = "user_group7.jsp";
    strConcatField = "ORG=" + lv_value.getAttribute("ORG") + "&ORG_NAME=" + lv_value.getAttribute("ORG_NAME") + "&PROJECT_CODE=" + lv_value.getAttribute("PROJECT_CODE") + "&USER_GROUP=" + lv_value.getAttribute("USER_GROUP");
...
<body onLoad="MM_preloadImages('images/btt_save2_over.gif','images/btt_back_over.gif');window_onload();" onresize="set_background()">
...
<%
    if( bolnSuccess ) {
        ...
        while( con.nextRecordElement() ) {
            ...
            con2.addData( "USER_GROUP",   "String", strUserGroupKey );
            con2.addData( "ORG",          "String", strFieldOrgCodeData );
            strScript = "ORG=\"" + strFieldOrgCodeData + "\" ORG_NAME=\"" + strFieldOrgNameData + "\" PROJECT_CODE=\"" + strProjectCodeKey + "\" USER_GROUP=\"" + strUserGroupKey + "\" ";
			strChkBox = "<input type=\"checkbox\" id=\"chkAllDocument" +intSeq+ "\" name=\"chkAllDocument" +intSeq +"\" onclick=\"chkbxClickValue('1')\" >";
			
            bolnSuccess2 = con2.executeService( strContainerName, strClassName, "countUserGroupOrg" );
            if( bolnSuccess2 ) {
				//strFieldCountData = con2.getColumn( "COUNT" );
...
<%
    if( bolnSuccess3 ) {
        ...
        while( con3.nextRecordElement() ) {
            ...
            con4.addData( "USER_GROUP",    "String", strUserGroupKey );
            con4.addData( "DOCUMENT_TYPE", "String", strFieldDocTypeData );
            bolnSuccess4 = con4.executeService( strContainerName, strClassName, "countDocumentTypeGroup" );
...
<%
 ...
 %>
			...
			<input type="hidden" id="hidConDocument"  name="hidConDocument"  value="">
...
var obj_hidConDocument   = document.getElementById( "hidConDocument" );
