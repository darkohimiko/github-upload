<%@page import="com.scc.edms.conf.ImageConfUtil"%>
<%@ page contentType="text/html; charset=tis-620"%>
<%@ page import="com.scc.security.UserInfo"%>
<%@ include file="inc/checkUser.jsp" %>
<%@ include file="inc/constant.jsp" %>
<%@ include file="inc/label.jsp" %>
<%@ include file="inc/utils.jsp" %>
<jsp:useBean id="con" scope="session" class="edms.cllib.EABConnector"/>
<jsp:useBean id="con1" scope="session" class="edms.cllib.EABConnector"/>
<%
    con.setRemoteServer("EAS_SERVER");
    con1.setRemoteServer("EAS_SERVER");

    UserInfo userInfo = (UserInfo)session.getAttribute( "USER_INFO" );
    String  strUserId = userInfo.getUserId();

    String screenname   = getField(request.getParameter("screenname"));
    String strClassName = "USER_ROLE";
    String strMode      = getField(request.getParameter("MODE"));

    boolean bolnSuccess  = false;
    boolean bolnSuccess1 = false;
    
    String strmsg          = "";
    String strCurrentDate  = "";
    String strVersionLang  = ImageConfUtil.getVersionLang();

    if( strVersionLang.equals("thai") ) {
     	strCurrentDate = getServerDateThai();
    }else {
        strCurrentDate = getServerDateEng();
    }
	
    String strUserRoleKey = getField(request.getParameter( "USER_ROLE_KEY" ));
    String strRoleNameKey = getField(request.getParameter( "ROLE_NAME_KEY" ));
    String strDefRoleKey  = getField(request.getParameter( "DEFAULT_ROLE_KEY" ));

    String strUserRoleDel = getField(request.getParameter( "USER_ROLE_DEL" ));

    String strUserRoleData         = getField(request.getParameter( "USER_ROLE_KEY" ));
    String strApplicationData      = getField(request.getParameter( "hidApplication" ));
    String strPermitFuncData       = getField(request.getParameter( "hidPermitFunc" ));
    String strApplicationGroupData = getField(request.getParameter( "hidAppGroup" ));
    String strApplicationNameData  = "";
    String strFunctionAvalData     = "";
    String strPermitFunctionData   = "";
    
    String strScript    = "";
    String strbttRole   = "";

    if( strMode == null || strMode.equals("")){
        strMode = "SEARCH";
    }
    
    if( strMode.equals("DELETE") ) {
    	con.addData( "USER_ROLE", "String", strUserRoleDel );
        bolnSuccess = con.executeService( strContainerName, strClassName, "deleteUserRole" );
        if( !bolnSuccess ){
            strmsg  = "showMsg(0,0,\" " + lc_can_not_delete_data + "\")";
            strMode = "SEARCH";
        }else{
            strmsg  = "showMsg(0,0,\" " + lc_delete_data_successfull + "\")";
            strMode = "SEARCH";
        }
    }
    
    if( strMode.equals("PERMIT") ) {
    	con.addData( "USER_ROLE",         "String", strUserRoleData );
    	con.addData( "APPLICATION",       "String", strApplicationData );
    	con.addData( "FUNCTION_AVAL",     "String", strPermitFuncData );
    	con.addData( "APPLICATION_GROUP", "String", strApplicationGroupData );
        con.addData( "ADD_USER",          "String", strUserId);
        con.addData( "ADD_DATE",          "String", strCurrentDate);
        con.addData( "UPD_USER",          "String", strUserId);
        bolnSuccess = con.executeService( strContainerName, strClassName, "commitRightsUserRole" );
        if( !bolnSuccess ){
            strmsg  = "showMsg(0,0,\" " + lc_can_not_update_data + "\")";
            strMode = "SEARCH";
        }else{
            strmsg  = "showMsg(0,0,\" " + lc_update_data_successfull + "\")";
            strMode = "SEARCH";
        }
    }
    
    if( strMode.equals("UNPERMIT") ) {
    	con.addData( "USER_ROLE",   "String", strUserRoleData );
    	con.addData( "APPLICATION", "String", strApplicationData );
        bolnSuccess = con.executeService( strContainerName, strClassName, "cancelRightUserRole" );
        if( !bolnSuccess ){
            strmsg  = "showMsg(0,0,\" " + lc_can_not_update_data + "\")";
            strMode = "SEARCH";
        }else{
            strmsg  = "showMsg(0,0,\" " + lc_update_data_successfull + "\")";
            strMode = "SEARCH";
        }
    }
    
    if( strMode.equals("ALLPERMIT") ) {
    	con.addData( "USER_ROLE", "String", strUserRoleData );
        con.addData( "ADD_USER",  "String", strUserId);
        con.addData( "ADD_DATE",  "String", strCurrentDate);
        con.addData( "UPD_USER",  "String", strUserId);
        bolnSuccess = con.executeService( strContainerName, strClassName, "allRightUserRole" );
        if( !bolnSuccess ){
            strmsg  = "showMsg(0,0,\" " + lc_can_not_update_data + "\")";
            strMode = "SEARCH";
        }else{
            strmsg  = "showMsg(0,0,\" " + lc_update_data_successfull + "\")";
            strMode = "SEARCH";
        }
    }
    
    if( strMode.equals("UNALLPERMIT") ) {
    	con.addData( "USER_ROLE", "String", strUserRoleData );
        bolnSuccess = con.executeService( strContainerName, strClassName, "cancelAllRightUserRole" );
        if( !bolnSuccess ){
            strmsg  = "showMsg(0,0,\" " + lc_can_not_update_data + "\")";
            strMode = "SEARCH";
        }else{
            strmsg  = "showMsg(0,0,\" " + lc_update_data_successfull + "\")";
            strMode = "SEARCH";
        }
    }
    
    if( strMode.equals("SEARCH") ) {
        bolnSuccess = con.executeService( strContainerName, strClassName, "findAllApplication" );
    }
    
%>

<!DOCTYPE HTML>
<html>
<head>
<title><%=lc_site_name%></title>
<meta http-equiv="Content-Type" content="text/html; charset=windows-874">
<script language="JavaScript" type="text/JavaScript">
<!--
function MM_swapImgRestore() { //v3.0
  var i,x,a=document.MM_sr; for(i=0;a&&i<a.length&&(x=a[i])&&x.oSrc;i++) x.src=x.oSrc;
}

function MM_preloadImages() { //v3.0
  var d=document; if(d.images){ if(!d.MM_p) d.MM_p=new Array();
    var i,j=d.MM_p.length,a=MM_preloadImages.arguments; for(i=0; i<a.length; i++)
    if (a[i].indexOf("#")!=0){ d.MM_p[j]=new Image; d.MM_p[j++].src=a[i];}}
}

function MM_findObj(n, d) { //v4.01
  var p,i,x;  if(!d) d=document; if((p=n.indexOf("?"))>0&&parent.frames.length) {
    d=parent.frames[n.substring(p+1)].document; n=n.substring(0,p);}
  if(!(x=d[n])&&d.all) x=d.all[n]; for (i=0;!x&&i<d.forms.length;i++) x=d.forms[i][n];
  for(i=0;!x&&d.layers&&i<d.layers.length;i++) x=MM_findObj(n,d.layers[i].document);
  if(!x && d.getElementById) x=d.getElementById(n); return x;
}

function MM_swapImage() { //v3.0
  var i,j=0,x,a=MM_swapImage.arguments; document.MM_sr=new Array; for(i=0;i<(a.length-2);i+=3)
   if ((x=MM_findObj(a[i]))!=null){document.MM_sr[j++]=x; if(!x.oSrc) x.oSrc=x.src; x.src=a[i+2];}
}
//-->
</script>
<script language="JavaScript" src="js/jquery-1.9.1.js"></script>
<script language="JavaScript" src="js/label/lb_user_role.js"></script>
<script language="JavaScript" src="js/util.js"></script>
<script language="JavaScript" src="js/constant.js"></script>
<script language="JavaScript" type="text/javascript">
<!--

var defaultRole = "<%=strDefRoleKey %>";

$(document).ready(window_onload);

function window_onload() {
    set_label();
    set_init(); 
    set_message(); 
}

function set_label(){
    $("#lb_function_main").text(lbl_function_main);
    $("#lb_sub_function").text(lbl_sub_function);
    $("#lb_save_data").text(lbl_save_data);
    $("#lb_edit_data").text(lbl_edit_data);
    $("#lb_delete_data").text(lbl_delete_data);
    $("#lb_search_data").text(lbl_search_data);
    $("#lb_print_document").text(lbl_print_document);
    $("#lb_edit_document").text(lbl_edit_document);
    $("#lb_delete_document").text(lbl_delete_document);
    $("#lb_censor_zone").text(lbl_censor_zone);
    $("#lb_annotations").text(lbl_annotations);
    $("#lb_link_document").text(lbl_link_document);
    $("#lb_OCR").text(lbl_OCR);
}

function set_init(){
    if( defaultRole == "Y" ) {
        $("#permit").hide();
        $("#unPermit").hide();
        $("#allPermit").hide();
        $("#unAllPermit").hide();
    }
}

function set_message(){
    
<%  if (!strmsg.equals("")) {  %>
    <%=strmsg%>
<%  }   %>
}

function buttonClick( lv_strMethod ){
    switch( lv_strMethod ){
        case "permit" :
            getPermit();
            if( $("#hidPermitFunc").val() == "" ) {
                alert( lc_not_permit_right );
                return;
            }
            $("#form1").attr('action', "user_role3.jsp");
            $("#form1").attr('target', "_self");
            $("#MODE").val("PERMIT");
            $("#form1").submit();
            break;
        case "unPermit" :
            if( $("#hidApplication").val() == "" ) {
                alert( lc_not_selected_function );
                return;
            }                
            getPermit();
            if( $("#hidPermitFunc").val() != "" ) {
                alert( lc_not_cancel_right );
                return;
            }
            $("#form1").attr('action', "user_role3.jsp");
            $("#form1").attr('target', "_self");
            $("#MODE").val("UNPERMIT");
            $("#form1").submit();
            break;
        case "allPermit" :
            $("#form1").attr('action', "user_role3.jsp");
            $("#form1").attr('target', "_self");
            $("#MODE").val("ALLPERMIT");
            $("#form1").submit();
            break;
        case "unAllPermit" :
            $("#form1").attr('action', "user_role3.jsp");
            $("#form1").attr('target', "_self");
            $("#MODE").val("UNALLPERMIT");
            $("#form1").submit();
            break;
        case "cancel" :
            $("#form1").attr('action', "user_role1.jsp");
            $("#form1").attr('target', "_self");
            $("#MODE").val("SEARCH");
            $("#form1").submit();
            break;
	}
}

function mouseOver( rowObjOver ) {
    if( rowObjOver.style.backgroundColor != '#b0e0e6' || rowObjOver.style.backgroundColor != 'rgb(176, 224, 230)' ) {
		rowObjOver.style.backgroundColor = '#9999ff';
	}
}

function mouseOut( rowObjOut, intRow ){
    
    if( intRow % 2 != 0 ) {
        if( rowObjOut.style.backgroundColor != '#b0e0e6' && rowObjOut.style.backgroundColor != 'rgb(176, 224, 230)' ) {
            rowObjOut.style.backgroundColor = '#FFFFFF';
        }
    }else {
        if( rowObjOut.style.backgroundColor != '#b0e0e6' && rowObjOut.style.backgroundColor != 'rgb(176, 224, 230)') {
            rowObjOut.style.backgroundColor = '#f9eac7';
        }
    }
}

function mouseClick( rowObjClick, intRow ) {
    var functionAval      = rowObjClick.getAttribute("FUNCTION_AVAL");
    var functionAvalArr   = functionAval.split(",");
    var avalArrLength     = functionAvalArr.length;
    var permitFunction    = rowObjClick.getAttribute("PERMIT_FUNCTION");
    var permitFunctionArr = permitFunction.split(",");
    var permitArrLength   = permitFunctionArr.length;
    var lastRow           = $("#lastRowSelected").val();
    
    if( lastRow != "" ) {
        if( lastRow % 2 != 0 ) {
            $("#tableResult tr").eq(lastRow).css('background-color','#ffffff');
        }else {
            $("#tableResult tr").eq(lastRow).css('background-color','#f9eac7');
        }
    }
    
    rowObjClick.style.backgroundColor = '#b0e0e6';
    $("#tableDataCheck").show();
    $("#lastRowSelected").val(intRow);
    
    // Display Check Box
    hideAllTr();
    chkAllFunction( "unChecked" );
    for(var i=0; i<avalArrLength; i++) {
    	if( functionAvalArr[i] == "insert" ) {
            $("#trSave").show();
    	}
    	if( functionAvalArr[i] == "update" ) {
                $("#trUpdate").show();
    	}
    	if( functionAvalArr[i] == "delete" ) {
                $("#trDel").show();
    	}
    	if( functionAvalArr[i] == "search" ) {
                $("#trSearch").show();
    	}
    	if( functionAvalArr[i] == "prnImg" ) {
                $("#trPrnDoc").show();
    	}
    	if( functionAvalArr[i] == "modImg" ) {
                $("#trEditDoc").show();
    	}
    	if( functionAvalArr[i] == "delImg" ) {
                $("#trDelDoc").show();
    	}
    	if( functionAvalArr[i] == "censor" ) {
                $("#trCencor").show();
    	}
    	if( functionAvalArr[i] == "anot" ) {
                $("#trAnnot").show();
    	}
    	if( functionAvalArr[i] == "link" ) {
                $("#trLinkDoc").show();
    	}
    	if( functionAvalArr[i] == "ocr" ) {
                $("#trOCR").show();
    	}
    }
    if( defaultRole == "N" ) {
    	$("#trButton").show();
    }
    if( permitFunction == "null" || permitFunction == null || permitFunction == "" ) {
    	permitArrLength = 0;
    }
    
    for(var i=0; i<permitArrLength; i++) {
        if( permitFunctionArr[i] == "insert" ) {
                $("#chkSave").prop('checked', true);
        }
        if( permitFunctionArr[i] == "update" ) {
                $("#chkUpdate").prop('checked', true);
        }
        if( permitFunctionArr[i] == "delete" ) {
                $("#chkDel").prop('checked', true);
        }
        if( permitFunctionArr[i] == "search" ) {
                $("#chkSearch").prop('checked', true);
        }
        if( permitFunctionArr[i] == "modImg" ) {
                $("#chkEditDoc").prop('checked', true);
        }
        if( permitFunctionArr[i] == "prnImg" ) {
                $("#chkPrnDoc").prop('checked', true);
        }
        if( permitFunctionArr[i] == "delImg" ) {
                $("#chkDelDoc").prop('checked', true);
        }
        if( permitFunctionArr[i] == "censor" ) {
                $("#chkCencor").prop('checked', true);
        }
        if( permitFunctionArr[i] == "anot" ) {
                $("#chkAnnot").prop('checked', true);
        }
        if( permitFunctionArr[i] == "link" ) {
                $("#chkLinkDoc").prop('checked', true);
        }
        if( permitFunctionArr[i] == "ocr" ) {
                $("#chkOCR").prop('checked', true);
        }
    }

    $("#hidAppGroup").val(rowObjClick.getAttribute("APPLICATION_GROUP"));
    $("#hidApplication").val(rowObjClick.getAttribute("APPLICATION"));
    getPermit();
}

function hideAllTr() {    
    $("#trSave").hide();
    $("#trUpdate").hide();
    $("#trDel").hide();
    $("#trSearch").hide();
    $("#trPrnDoc").hide();
    $("#trEditDoc").hide();
    $("#trDelDoc").hide();
    $("#trCencor").hide();
    $("#trAnnot").hide();
    $("#trLinkDoc").hide();
    $("#trOCR").hide();
    $("#trButton").hide();
}

function clearAllCheckBox() {
    $("#chkSave").prop('checked', false);
    $("#chkUpdate").prop('checked', false);
    $("#chkDel").prop('checked', false);
    $("#chkSearch").prop('checked', false);
    $("#chkEditDoc").prop('checked', false);
    $("#chkPrnDoc").prop('checked', false);
    $("#chkDelDoc").prop('checked', false);
    $("#chkCencor").prop('checked', false);
    $("#chkAnnot").prop('checked', false);
    $("#chkLinkDoc").prop('checked', false);
    $("#chkOCR").prop('checked', false);
}

function chkAllFunction( strValue ){
    if( strValue == "checked" ) {
        if( $("#trSave").css('display') == 'inline' ) {
                $("#chkSave").prop('checked', false);
        }else {
                $("#chkSave").prop('checked', true);
        }
        if( $("#trUpdate").css('display') == 'inline' ) {
                $("#chkUpdate").prop('checked', false);
        }else {
                $("#chkUpdate").prop('checked', true);
        }
        if( $("#trDel").css('display') == 'inline' ) {
                $("#chkDel").prop('checked', false);
        }else {
                $("#chkDel").prop('checked', true);
        }
        if( $("#trSearch").css('display') == 'inline' ) {
                $("#chkSearch").prop('checked', false);
        }else {
                $("#chkSearch").prop('checked', true);
        }
        if( $("#trEditDoc").css('display') == 'inline' ) {
                $("#chkEditDoc").prop('checked', false);
        }else {
                $("#chkEditDoc").prop('checked', true);
        }
        if( $("#trPrnDoc").css('display') == 'inline' ) {
                $("#chkPrnDoc").prop('checked', false);
        }else {
                $("#chkPrnDoc").prop('checked', true);
        }
        if( $("#trDelDoc").css('display') == 'inline' ) {
                $("#chkDelDoc").prop('checked', false);
        }else {
                $("#chkDelDoc").prop('checked', true);
        }
        if( $("#trCencor").css('display') == 'inline' ) {
                $("#chkCencor").prop('checked', false);
        }else {
                $("#chkCencor").prop('checked', true);
        }
        if( $("#trAnnot").css('display') == 'inline' ) {
                $("#chkAnnot").prop('checked', false);
        }else {
                $("#chkAnnot").prop('checked', true);
        }
        if( $("#trLinkDoc").css('display') == 'inline' ) {
                $("#chkLinkDoc").prop('checked', false);
        }else {
                $("#chkLinkDoc").prop('checked', true);
        }
        if( $("#trOCR").css('display') == 'inline' ) {
                $("#chkOCR").prop('checked', false);
        }else {
                $("#chkOCR").prop('checked', true);
        }
    }else {
        $("#chkSave").prop('checked', false);
        $("#chkUpdate").prop('checked', false);
        $("#chkDel").prop('checked', false);
        $("#chkSearch").prop('checked', false);
        $("#chkEditDoc").prop('checked', false);
        $("#chkPrnDoc").prop('checked', false);
        $("#chkDelDoc").prop('checked', false);
        $("#chkCencor").prop('checked', false);
        $("#chkAnnot").prop('checked', false);
        $("#chkLinkDoc").prop('checked', false);
        $("#chkOCR").prop('checked', false);
    }
}

function uncheckAllChecked() {
	$("#chkAllSub").prop('checked', false);
}

function getPermit() {
	var permitFunc = "";
	if( $("#chkSave").prop('checked') == true ) {
		permitFunc += "insert,";
	}
	if( $("#chkUpdate").prop('checked') == true ) {
		permitFunc += "update,";
	}
	if( $("#chkDel").prop('checked') == true ) {
		permitFunc += "delete,";
	}
	if( $("#chkSearch").prop('checked') == true ) {
		permitFunc += "search,";
	}
	if( $("#chkEditDoc").prop('checked') == true ) {
		permitFunc += "modImg,";
	}
	if( $("#chkPrnDoc").prop('checked') == true ) {
		permitFunc += "prnImg,";
	}
	if( $("#chkDelDoc").prop('checked') == true ) {
		permitFunc += "delImg,";
	}
	if( $("#chkCencor").prop('checked') == true ) {
		permitFunc += "censor,";
	}
	if( $("#chkAnnot").prop('checked') == true ) {
		permitFunc += "anot,";
	}
	if( $("#chkLinkDoc").prop('checked') == true ) {
		permitFunc += "link,";
	}
	if( $("#chkOCR").prop('checked') == true ) {
		permitFunc += "ocr,";
	}
	if( permitFunc.length > 0 ){
		permitFunc = permitFunc.substr( 0, permitFunc.lastIndexOf(",") );
	}
	$("#hidPermitFunc").val(permitFunc);
}

//-->
</script>
<link href="css/edas.css" type="text/css" rel="stylesheet">
</head>
<body onLoad="MM_preloadImages('images/btt_all-right_over.gif','images/btt_cancelall-right_over.gif','images/btt_cancelright_over.gif',
			'images/btt_commitrights_over.gif','images/btt_back_over.gif','images/btt_selectall_s_over.gif','images/btt_noselect_over.gif');">
<form id="form1" name="form1" method="post" action="" >
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td valign="top">
            <table width="700" border="0" cellspacing="0" cellpadding="0">
                <tr>
                    <td height="25" class="label_header01" colspan="3">&nbsp;&nbsp;<%=screenname%>&nbsp; >> &nbsp;<%=strRoleNameKey %></td>
                </tr>
                <tr>
                    <td height="25" class="label_header01" colspan="3"><BR></td>
                </tr>
                <tr> 
                    <td valign="top"><div align="center"> 
                        <table id="tableResult" width="250" border="0" cellpadding="0" cellspacing="0">
                            <tr class="hd_table">
                                <td width="10"><img src="images/hd_tb_01.gif" width="10" height="28"></td>
                                <td width="30">&nbsp;</td>
                                <td width="210" align="center">
                                    <div align="center"><span id="lb_function_main"></span></div>
                                </td>
                                <td align="right" width="10"><img src="images/hd_tb_04.gif" width="10" height="28"></td>
                            </tr>
<%
    if( (bolnSuccess) ) {
        int intSeq = 1;
        int idx = 0;
        while( con.nextRecordElement() ){
            strApplicationData      = con.getColumn( "APPLICATION" );
            strApplicationNameData  = con.getColumn( "APPLICATION_NAME" );
            strFunctionAvalData     = con.getColumn( "FUNCTION_AVAL" );
            strApplicationGroupData = con.getColumn( "APPLICATION_GROUP" );
            
            con1.addData( "USER_ROLE",   "String", strUserRoleKey );
            con1.addData( "APPLICATION", "String", strApplicationData );
    		bolnSuccess1 = con1.executeService( strContainerName, strClassName, "findRoleApplication" );
    		
            if( bolnSuccess1 ) {
            	strbttRole = "<img src=\"images/True.gif\" name=\"role" + intSeq + " \"  width=\"16\" height=\"16\" border=\"0\" id=\"role\">";
            }else {
            	strbttRole = "<img src=\"images/False.gif\" name=\"role" + intSeq + " \"  width=\"16\" height=\"16\" border=\"0\" id=\"role\">";
            }
            strPermitFunctionData = con1.getHeader( "PERMIT_FUNCTION" );
            
            strScript    = "APPLICATION=\"" + strApplicationData + "\" APPLICATION_NAME=\"" + strApplicationNameData + "\" ";
            strScript    += "FUNCTION_AVAL=\"" + strFunctionAvalData + "\" PERMIT_FUNCTION=\"" + strPermitFunctionData + "\" APPLICATION_GROUP=\"" + strApplicationGroupData + "\" ";
			
            idx = 2 - (intSeq % 2);
%>                      
                            <tr class="table_data<%=idx%>" onmouseover="mouseOver(this);" onmouseout="mouseOut(this,'<%=intSeq %>');"
                                onclick="mouseClick(this,'<%=intSeq %>');" <%=strScript %>>
                                <td width="10">&nbsp;</td>
                                <td width="30"><%=strbttRole %></td>
                                <td colspan="2"><div align="left">&nbsp;<%=strApplicationNameData %></div></td>
                            </tr>
<%
            intSeq++;
        }
     }
 %>                      
                        </table>
                    </div>
                    </td>
                    <td width="15">&nbsp;</td>
                    <td valign="top"><div align="center"> 
                        <table width="250" border="0" cellpadding="0" cellspacing="0">
                            <tr class="hd_table"> 
                                <td width="10"><img src="images/hd_tb_01.gif" width="10" height="28"></td>
                                <td width="230" align="center">
                                    <div align="center"><span id="lb_sub_function"></span></div>
                                </td>
                                <td align="right" width="10"><img src="images/hd_tb_04.gif" width="10" height="28"></td>
                            </tr>
                            <tr class="table_data1">
                            <td>&nbsp;</td>
                            <td colspan="2">
                                <table id="tableDataCheck" width="235" border="0" cellpadding="0" cellspacing="0" style="display: none">
                                    <tr id="trSave" class="table_data1">
                                        <td align="left">
                                            <div><input type="checkbox" id="chkSave" >&nbsp;<span id="lb_save_data"></span></div>
                                        </td>
                                    </tr>
                                    <tr id="trUpdate" class="table_data1">
                                        <td align="left">
                                            <div><input type="checkbox" id="chkUpdate" >&nbsp;<span id="lb_edit_data"></span></div>
                                        </td>
                                    </tr>
                                    <tr id="trDel" class="table_data1">
                                        <td align="left">
                                            <div><input type="checkbox" id="chkDel" >&nbsp;<span id="lb_delete_data"></span></div>
                                        </td>
                                    </tr>
                                    <tr id="trSearch" class="table_data1">
                                        <td align="left">
                                            <div><input type="checkbox" id="chkSearch" >&nbsp;<span id="lb_search_data"></span></div>
                                        </td>
                                    </tr>
                                    <tr id="trPrnDoc" class="table_data1">
                                        <td align="left">
                                            <div><input type="checkbox" id="chkPrnDoc" >&nbsp;<span id="lb_print_document"></span></div>
                                        </td>
                                    </tr>
                                    <tr id="trEditDoc" class="table_data1">
                                        <td align="left">
                                            <div><input type="checkbox" id="chkEditDoc" >&nbsp;<span id="lb_edit_document"></span></div>
                                        </td>
                                    </tr>
                                    <tr id="trDelDoc" class="table_data1">
                                        <td align="left">
                                            <div><input type="checkbox" id="chkDelDoc" >&nbsp;<span id="lb_delete_document"></span></div>
                                        </td>
                                    </tr>
                                    <tr id="trCencor" class="table_data1">
                                        <td align="left">
                                            <div><input type="checkbox" id="chkCencor" >&nbsp;<span id="lb_censor_zone"></span></div>
                                        </td>
                                    </tr>
                                    <tr id="trAnnot" class="table_data1">
                                        <td align="left">
                                            <div><input type="checkbox" id="chkAnnot" >&nbsp;<span id="lb_annotations"></span></div>
                                        </td>
                                    </tr>
                                    <tr id="trLinkDoc" class="table_data1">
                                        <td align="left">
                                            <div><input type="checkbox" id="chkLinkDoc" >&nbsp;<span id="lb_link_document"></span></div>
                                        </td>
                                    </tr>
                                    <tr id="trOCR" class="table_data1">
                                        <td align="left">
                                            <div><input type="checkbox" id="chkOCR" >&nbsp;<span id="lb_OCR"></span></div>
                                        </td>
                                    </tr>
                                    <tr id="trButton" class="table_data1">
                                        <td align="center">
                                            <a href="#" onclick= "chkAllFunction('checked')" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('checked','','images/btt_selectall_s_over.gif',1)">
                                                <img src="images/btt_selectall_s.gif" id="checked" name="checked" width="100" height="20" border="0"></a>&nbsp;
                                            <a href="#" onclick= "chkAllFunction('unChecked')" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('unChecked','','images/btt_noselect_over.gif',1)">
                                                <img src="images/btt_noselect.gif" id="unChecked" name="unChecked" width="100" height="20" border="0"></a>
                                        </td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
                    </table>
                    </div>
                    </td>
                </tr>
                <tr>
                    <td height="25" class="label_header01" colspan="3"><BR></td>
                </tr>
                <tr>
                    <td height="25" class="label_header01" colspan="3" align="center">
                    <a href="#" onclick= "buttonClick('permit')" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('permit','','images/btt_commitrights_over.gif',1)">
                    	<img src="images/btt_commitrights.gif" id="permit" name="permit" width="67" height="22" border="0"></a>
                    <a href="#" onclick= "buttonClick('unPermit')" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('unPermit','','images/btt_cancelright_over.gif',1)">
                    	<img src="images/btt_cancelright.gif" id="unPermit" name="unPermit" width="102" height="22" border="0"></a>&nbsp;&nbsp;&nbsp;
                    <a href="#" onclick= "buttonClick('allPermit')" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('allPermit','','images/btt_all-right_over.gif',1)">
                    	<img src="images/btt_all-right.gif" id="allPermit" name="allPermit" width="102" height="22" border="0"></a>
                    <a href="#" onclick= "buttonClick('unAllPermit')" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('unAllPermit','','images/btt_cancelall-right_over.gif',1)">
                    	<img src="images/btt_cancelall-right.gif" id="unAllPermit" name="unAllPermit" width="142" height="22" border="0"></a>&nbsp;&nbsp;&nbsp;
                    <a href="#" onclick= "buttonClick('cancel')" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('back','','images/btt_back_over.gif',1)">
                        <img src="images/btt_back.gif" name="back" width="67" height="22" border="0"></a>
                    </td>
                </tr>
            </table>
        </td>
    </tr>
</table>
<input type="hidden" id="MODE"             name="MODE"             value="<%=strMode%>">
<input type="hidden" id="screenname"       name="screenname"       value="<%=screenname%>">
<input type="hidden" id="USER_ROLE_DEL"    name="USER_ROLE_DEL"    value="">
<input type="hidden" id="lastRowSelected"  name="lastRowSelected"  value="">
<input type="hidden" id="USER_ROLE_KEY"    name="USER_ROLE_KEY"    value="<%=strUserRoleKey %>">
<input type="hidden" id="ROLE_NAME_KEY"    name="ROLE_NAME_KEY"    value="<%=strRoleNameKey %>">
<input type="hidden" id="DEFAULT_ROLE_KEY" name="DEFAULT_ROLE_KEY" value="<%=strDefRoleKey %>">
<input type="hidden" id="hidApplication"   name="hidApplication"   value="">
<input type="hidden" id="hidPermitFunc"    name="hidPermitFunc"    value="">
<input type="hidden" id="hidAppGroup"      name="hidAppGroup"      value="">                        
</form>
</body>
</html>
