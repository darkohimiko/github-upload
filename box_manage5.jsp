...
<%
    ...
    String strDocUserCon   = checkNull(request.getParameter("hidDocumentUserCon"));
    ...
    String strConcatBatchno         = checkNull( request.getParameter( "CONCAT_BATCH_NO" ) );
    String strConcatDocumentRunning = checkNull( request.getParameter( "CONCAT_DOCUMENT_RUNNING" ) );
    ...
    String strInsertBatchno[]         = {};
    String strInsertDocumentRunning[] = {};
    ...
    boolean bolSuccessAddDetail  = false;
    ...
    con.addData( "PROJECT_CODE" , "String" , strProjectCode );
    con.addData( "YEAR"         , "String" , strYear );
    con.addData( "BOX_SEQN"     , "String" , strBoxSeqn );
    bolSuccess = con.executeService(strContainerName, "BOX_MANAGE", "findDocNoInBoxDetail");
    if(bolSuccess){
        ...
        while(con.nextRecordElement()){
            ...
            if(cnt == 0){
                strAllDocumentNo += "'" + con.getColumn("DOCUMENT_NO") + "'";
            }else{
                strAllDocumentNo += ",'" + con.getColumn("DOCUMENT_NO") + "'";
            }
    ...
    if(strMethod.equals("ADD_DATA")){
        ...
        strInsertBatchno         = strConcatBatchno.split( "</>" );
        strInsertDocumentRunning = strConcatDocumentRunning.split( "</>" );
        
        if( strConcatBatchno.length() > 0 ){
            for( int intArr = 0 ; intArr < strInsertBatchno.length ; intArr++ ){
                con.addData( "DETAIL." + ( intArr + 1 ) + ".BATCH_NO" 		   , "String" , strInsertBatchno[ intArr ] );
                con.addData( "DETAIL." + ( intArr + 1 ) + ".DOCUMENT_RUNNING"  , "String" , strInsertDocumentRunning[ intArr ] );
            }
        }

        bolSuccessAddDetail = con.executeService(strContainerName, "BOX_MANAGE", "createBoxDetail");
        if( !bolSuccessAddDetail ){
            strErrorMessage = lc_cannot_add_data_box;
        }
    ...
    if( strAccessType.equals("A") ) {
        ...
        if( strSecurityFlag.equals("I") ) {
            ...
            con2.addData( "PROJECT_CODE", strProjectCode );
            con2.addData( "USER_ID",      strUserId );
            bolnAccType = con2.executeService( strContainerName, strClassName, "countUserAccessByOwnerUser" );
            if( bolnAccType ) {
                while( con2.nextRecordElement() ) {
                        strCount = con2.getColumn( "CNT" );
                }
            }
            ...
            con2.addData( "PROJECT_CODE", strProjectCode );
            con2.addData( "USER_GROUP",   strUserGroup );
            bolnAccType = con2.executeService( strContainerName, strClassName, "countGroupAccessByOwnerUser" );
            if( bolnAccType ) {
                while( con2.nextRecordElement() ) {
                    strCount = con2.getColumn( "CNT" );
                }
    ...
    if(!strAllDocumentNo.equals("")){
        strSQLcondition += " AND MASTER_SCAN_" + strProjectCode + ".DOCUMENT_NO NOT IN (" + strAllDocumentNo +  ")";
    }
...
function click_save(){
    ...
    if( objCheckboxSelect.length ){
        for( var intCount = 0 ; intCount < objCheckboxSelect.length ; intCount++ ){
            ...
            if( objCheckboxSelect[ intCount ].checked ){
                ...
                if( objLabelSelect ){
                    strConcatBatchno 	     += objLabelSelect.getAttribute("BATCH_NO") + "</>";
                    strConcatDocumentRunning += objLabelSelect.getAttribute("DOCUMENT_RUNNING") + "</>";
                }
            }
        }
    ...
    if( bolnCheck ){
        form1.CONCAT_BATCH_NO.value         = strConcatBatchno;
        form1.CONCAT_DOCUMENT_RUNNING.value = strConcatDocumentRunning;

        form1.OLD_METHOD.value = form1.METHOD.value;
        form1.METHOD.value = "ADD_DATA";
        form1.method = "post";
        form1.action = "box_manage5.jsp";
        form1.submit();
    }
...
function window_onload() {
    ...
    <% if(bolSuccessAddDetail){%>
    showMsg( 0 , 0 , "<%=lc_add_data_box_successful%>");
    opener.form1.submit();
    click_cancel();
...
<body onLoad="MM_preloadImages('images/txt_search_over.gif','images/txt_searchtotal_over.gif','images/txt_searchresult_over.gif','images/btt_new_over.gif','images/btt_search_over.gif','images/i_new_over.jpg','images/save2_over.gif','images/i_out_over.jpg');window_onload()" onunload="window_onunload()">
                    ...
                    <td height="62" background="images/inner_img_03.jpg"> 
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr> 
                                ...
                                <td width="600" valign="bottom"><%=strBttFunction %><a href="#" onclick="click_cancel()" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('i_out','','images/i_out_over.jpg',1)"><img src="images/i_out.jpg" name="i_out" width="56" height="62" border="0"></a></td>
...
<%
			...
			for(int arrHead=0; arrHead < arrNameHeader.length; arrHead++){
                            ...
                            if (arrTableZoom[arrHead].equals("number") || arrTableZoom[arrHead].equals("currency")) {
                                strHalign = "right";
                            } else if (arrTableZoom[arrHead].equals("date") || arrTableZoom[arrHead].equals("date_eng") || arrTableZoom[arrHead].equals("tin_pin")) {
                                strHalign = "center";
                            } else {
                                strHalign = "left";
                            }
...
%>				
                                <tr class="table_data1"> 
                                    <td colspan="<%=arrFieldIndex.length + 7 %>" align="center"><%=strErrorMessage %></td>
                                </tr>				
<% 				
			}else{
                            ...
                            String strDocumentRunningData = "";

                            while(con.nextRecordElement()){
                                strStoreFlag           = con.getColumn("STORE_FLAG");
                                strBatchNoData         = con.getColumn("BATCH_NO");
                                strDocumentRunningData = con.getColumn("DOCUMENT_RUNNING");

%>
				<tr class="table_data<%=(cnt%2)+1%>" > 
                                    <td>&nbsp;</td>
                                    <td><div align="center">
                                        <input type="checkbox" name="CHK_SELECT" onclick="check_not_select_all()">
                                    </div></td>
<%
                                String strAlign = "";
                                ...
                                for(int idx=0; idx<arrFieldIndex.length; idx++ ){
                                    ...
                                    if(!arrTableZoom[idx].equals("not")&&!arrTableZoom[idx].equals("date")&&!arrTableZoom[idx].equals("date_eng")&&!arrTableZoom[idx].equals("tin")&&!arrTableZoom[idx].equals("pin")&&!arrTableZoom[idx].equals("number")&&!arrTableZoom[idx].equals("currency")){
                                        ...
                                        if(arrTableZoom[idx].equals("date")){
                                            strDataIndex[idx] = dateToDisplay(con.getColumn(arrFieldIndex[idx]),"/");									
                                        }else if(arrTableZoom[idx].equals("date_eng")){
                                            strDataIndex[idx] = dateToDisplay(con.getColumn(arrFieldIndex[idx]),"/");									
                                    ...
                                    if (arrTableZoom[idx].equals("number") || arrTableZoom[idx].equals("currency")) {
                                        strAlign = "right";
                                    } else if (arrTableZoom[idx].equals("date") || arrTableZoom[idx].equals("date_eng") || arrTableZoom[idx].equals("tin_pin")) {
                                        strAlign = "center";
                                    } else {
                                        strAlign = "left";
                                    }
...
%>                                  <td align="right" width="20">
                                        <a href="#" onclick="open_detail(this)" PROJECT_CODE="<%=strProjectCode %>" PROJECT_NAME="<%=strProjectName %>" BATCH_NO="<%=strBatchNoData %>" DOCUMENT_RUNNING="<%=strDocumentRunningData %>" ><img src="images/page_detail.gif" name="detail<%=cnt+1 %>" width="16" height="16" border="0" style="cursor:pointer;" title="<%=lb_detail %>"></a>
                                    </td>
                                    <td align="right" width="20">	
<%
                                con2.addData("PROJECT_CODE",     "String", strProjectCode);
                                con2.addData("BATCH_NO",         "String", strBatchNoData);
                                con2.addData("DOCUMENT_RUNNING", "String", strDocumentRunningData);
                                boolean bolSuccessAttch = con2.executeService(strContainerName, strClassName, "hasAttach");
                                if(bolSuccessAttch){
%>
                                            <a href="javascript:open_clip('<%=strProjectCode %>','<%=strBatchNoData %>','<%=strDocumentRunningData %>','<%=strStoreFlag %>')" ><img src="images/page_attach.gif" name="attach<%=cnt+1 %>" width="16" height="16" border="0" style="cursor:pointer;" title="<%=lb_attachment %>"></a>
                            <%	}   %>
                                        <label id="LABEL_SELECT<%=cnt%>" BATCH_NO="<%=strBatchNoData%>" DOCUMENT_RUNNING="<%=strDocumentRunningData%>" ></label>
                                    </td>
                                    <td width="11">&nbsp;</td>
                                </tr>		                            
<%					
...
<input type="hidden" name="CONCAT_BATCH_NO" >
<input type="hidden" name="CONCAT_DOCUMENT_RUNNING" >

<input type="hidden" name="METHOD"    value="<%=strMethod%>">
<input type="hidden" name="OLD_METHOD" >
<input type="hidden" name="PERMIT_FUNCTION" >
